...
module.exports = {
  ats: [],
  addRoute: function addRoute(url, cb, wcb) {
  ...
  checkAts: function checkAts(url, socket) {
...
function checkCompatability(url, ats) {
  var match;
  if (!noSameParams(url)) {
    return { error: "param duplicates", msg: "A route cannot have multiple parameters with the same name" };
